...
<%
    ...
    if (isFlats) {
        ...
        if (addedProduct != null) {
            ...
            newProduct.setUser(Integer.parseInt((String) session.getValue("id")));
        ...
        String userFilter = (String) request.getParameter("userFilter");
        String next = (String) request.getParameter("next");

        if (next != null) {
            String pageNext = (String) session.getAttribute("MyListPage");
            int intPageNext = Integer.parseInt(pageNext);
            intPageNext++;
            pageNext = String.valueOf(intPageNext);
            session.setAttribute("MyListPage", pageNext);
        }
        ...
        String isPrevString = (String) session.getAttribute("MyListPage");
        ...
        ProductsRest prr = new ProductsRest();
        if (userFilter != null) {
            session.setAttribute("MyListFilter", userFilter);
            session.setAttribute("MyListPage", "1");
        }
        Response responseProduct2;
        if (session.getAttribute("MyListFilter") != null) {
            responseProduct = prr.getListbyGivenFlat((String) session.getValue("currentFlat"), (String) session.getAttribute("MyListFilter"), "3", (String) session.getAttribute("MyListPage"), (String) session.getValue("token"));
            String pageNext = (String) session.getAttribute("MyListPage");
            int intPageNext = Integer.parseInt(pageNext);
            intPageNext++;
            pageNext = String.valueOf(intPageNext);
            responseProduct2 = prr.getListbyGivenFlat((String) session.getValue("currentFlat"), (String) session.getAttribute("MyListFilter"), "3", pageNext, (String) session.getValue("token"));
            List<Product> products2 = (List<Product>) responseProduct2.getObject();
            if (products2.isEmpty()) {
                nextPage = false;
            }
        } else {
            responseProduct = prr.getListbyGivenFlat((String) session.getValue("currentFlat"), (String) session.getValue("id"), "3", (String) session.getAttribute("MyListPage"), (String) session.getValue("token"));
            String pageNext = (String) session.getAttribute("MyListPage");
            int intPageNext = Integer.parseInt(pageNext);
            intPageNext++;
            pageNext = String.valueOf(intPageNext);
            responseProduct2 = prr.getListbyGivenFlat((String) session.getValue("currentFlat"), (String) session.getValue("id"), "3", pageNext, (String) session.getValue("token"));
            List<Product> products2 = (List<Product>) responseProduct2.getObject();
            if (products2.isEmpty()) {
                nextPage = false;
            }
        ...
        if (productID != null) {
            ...
            if (productID.startsWith("r")) {
                ...
                for (Product product : products) {
                    if ((String) session.getAttribute("isOption") == String.valueOf(product.getId())) {
                        toReserve = product;
                    }
                }
                responseProduct = pr.unreserveProduct(toReserve, (String) session.getValue("token"));
                response.sendRedirect("MyList.jsp");
            }
...
<html>
    ...
    <body>
        <div class ="blur">
            ...
            <nav class="clearfix bottom-bar">
                <ul class="clearfix">
                    <li><a href="#" class="mainlistClick" ><%=lang.mainList%></a></li>
                    <li><a href="#" class="mylistClick active-bottom"><%=lang.myList%>    </a></li>
            ...
            <%
                String userFilter = (String) session.getAttribute("MyListFilter");
                if (userFilter == null) {
                    userFilter = (String) session.getAttribute("id");
                }
                String current = (String) session.getAttribute("id");
            %>

            <!--  Start List -->
            <% if (isFlats) {
                    if (!products.isEmpty() || products.isEmpty() && userFilter.equals(current)) {
            %>
            <div class ="outer">
                <section id="todoapp">

                    <%
                        if (userFilter.equals(current)) {
                    %>
                    <header id="header">
                    ...
                    <section id="main">
                        <ul id="todo-list">
                            ...
                            <li >
                                <div class="view">
                                    <label  class="todo-item <% if (userFilter.equals(current)) {%> <%=  "moreProduct"%>" <% } else {%> <%=  "infoProduct"%>"    <%}
            ...
            <div class="myModal modal-item">
                ...
                <div class="modal-options">
                    ...
                    <br>
                    <hr>
                    <a href="#" class="makeReservation"><%=lang.addToMainList%></a>
                    <br>
                    <hr>
    ...
    <script src="${pageContext.request.contextPath}/resources/js/modalsMyList.js"></script>
</html>
